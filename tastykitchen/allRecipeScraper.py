...
def appCheck(directions, currRecipe):
    for direction in directions:
        if "toaster oven" in direction:
            currRecipe['appliances']['toasterOven'] = True
        if "blend" in direction:
            currRecipe['appliances']['blender'] = True
        if "air fryer" in direction:
            currRecipe['appliances']['airFryer'] = True
        if "microwave" in direction:
            currRecipe['appliances']['microwave'] = True
        if "freeze" in direction:
            currRecipe['appliances']['freezer'] = True
        for keyword in ['stand-mixer', 'stand mixer', 'mixer']:
            if keyword in direction:
                currRecipe['appliances']['mixer'] = True
                break
        for keyword in ['bbq', 'barbeque', 'grill']:
            if keyword in direction:
                if 'barbeque sauce' in direction:
                    continue
                currRecipe['appliances']['bbq'] = True
                break

        # add checks for the last few applicanes

# Turns array of strings into dictionary given a seperator for key and values


def listSplitter(unsplitList, seperator):
    listKeys, listVals = map(
        list, zip(*(x.split(seperator) for x in unsplitList)))
    listKeys, listVals = [" ".join(val.split()) for val in listKeys], [
        " ".join(val.split()) for val in listVals]
...
for url in URLS:
    ...
    while nextURL:
        ...
        for recipe in recipeList:
            ...
            recipeSoup = BeautifulSoup(
                recipeResp.content, features="html.parser")
            ...
            title = recipeSoup.select_one(
                'div.intro.article-info div.headline-wrapper h1.headline.heading-content.elementFont__display').get_text()
            nutrition = [nutrition.get_text() for nutrition in recipeSoup.select(
                'div.recipeNutritionSectionBlock div.section-body')]
            timeTitles = [times.get_text() for times in recipeSoup.select(
                'div.two-subcol-content-wrapper div.recipe-meta-item')]
            imgLink = recipeSoup.select_one(
                'aside.recipe-tout-image div.component.lazy-image') or recipeSoup.select_one('div.image-container div.component.lazy-image')
        ...
        nextURL = soup.find('a', href=True, class_='category-page-list-related-load-more-button') or soup.find(
            'a', href=True, class_='categoryPage__relatedButton categoryPage__relatedButton--next')
